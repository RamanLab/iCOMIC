...
rule samtools_stats:
    input:
        "results/aligner_results/{sample}_{condition}_Rep{rep}_cutadapt.bam"
    output:
        "results/aligner_results/samtools-stats/{sample}_{condition}_Rep{rep}_cutadapt.txt"
    log:
        "logs/samtools-stats/{sample}_{condition}_Rep{rep}_cutadapt.logs"
    wrapper:
        "0.27.1/bio/samtools/stats"

rule multiqc:
    input:
        expand("results/aligner_results/samtools-stats/{sample}_{condition}_Rep{rep}_cutadapt.txt", sample=samples, condition=type, rep=reps)
    output:
