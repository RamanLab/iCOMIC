...
#        "0.27.1/bio/bcftools/call"
    shell:
        "bcftools mpileup -f {input.ref} {input.samples} | bcftools call -mv -o {output} 2> {log}" 
...
rule bcftools_merge:
    input:
        calls=expand("called/{sample}.{contig}.bcf.gz", sample=samples.index, contig=contigs),
#        index=expand("called/{sample}.{contig}.vcf.gz", sample=samples.index, contig=contigs) 
    output:
        "filtered/all.vcf"
    params:
        ""  # optional parameters for bcftools concat (except -o)
